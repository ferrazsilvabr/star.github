const ReactionModel = require("../ReactionRole");
...
module.exports = {
  ...
  run: async (bot, message, args) => {
    if (!message.guild.roles.cache.has(args[1]))
      return message.channel.send(`Esse Cargo nao existe.`);
    if (!args[0])
    ...
    ReactionModel.findOne(
      ...
      async (err, data) => {
        if (err) throw err;
        if (!data)
          return message.channel.send(`Isso não é uma mensagem!`);
        ...
        return message.channel.send(`Deletei o cargo por Reação!`);
      }
    );
  },
};