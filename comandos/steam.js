...
async function run(bot, message, args, Discord,client) {
    const token = "steamToken"; //I reset mine.
    if(!args[0]) return message.channel.send("Diga um Usuário!");
    const url = `http://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key=${token}&vanityurl=${args.join(" ")}`;

    fetch(url).then(res => res.json()).then(body => {
        if(body.response.success === 42) return message.channel.send("Não achei esse ");
