...
if (author !== null && timeout - (Date.now() - author) > 0) {
    ...
    .setColor("#FF0000")
    .setDescription(`:x: Você ja roubou alguém\n\nTente de novo em ${time.minutes}m ${time.seconds}s `);
    message.channel.send(timeEmbed)
  } else {

let moneyEmbed = new Discord.MessageEmbed()
  .setColor("#FF0000")
  .setDescription(`:x: VocÊ precisa de 200 StarCoins para roubar alguem`);
...
let moneyEmbed2 = new Discord.MessageEmbed()
  .setColor("#FF0000")
  .setDescription(`:x: ${user.user.username} não tem nada para roubar`);
if (targetuser < 0) {
...
.setDescription(`✅ Você roubou ${user} e conseguiu ${random} StarCoins`)
.setColor("#FF0000")
